...
// Used when a note (not a reply) asks for both a draft and a score in the
// same message. Deliberately narrower than REPLY_SCORE_TRIGGER below, to
// avoid false-triggering on ordinary use of "score"/"rate" in a business
// sense (e.g. "we scored a big client win this month").
const SCORE_TRIGGER =
...
// Used when the message is a reply to a specific post — the reply itself
// already disambiguates "what," so a bare "score" or "rate" is enough here
// without needing the fuller phrasing SCORE_TRIGGER requires.
const REPLY_SCORE_TRIGGER =
  /\bscores?\b|\brate[sd]?\b|\brating\b|\bworth\s+posting\b|\bgood\s+enough\s+to\s+post\b/i;
...
const PURE_SCORE_REQUEST =
  /^(score\s*(this|it)?|rate\s*(this|it)?|what'?s\s+the\s+score|how\s+(does|would|is)\s+this\s+(rate|score)|is\s+this\s+worth\s+posting|worth\s+posting|good\s+enough\s+to\s+post)[.!?\s]*$/i;

function isScoreTrigger(text) {
  return SCORE_TRIGGER.test(text);
}

function isReplyScoreTrigger(text) {
  return REPLY_SCORE_TRIGGER.test(text);
}
...
module.exports = { scorePost, isScoreTrigger, isReplyScoreTrigger, isPureScoreRequest };
